...
namespace ConnectorLib.usb2snes
{
    ...
    public class core
    {
        ...
        public core() => serialPort = new SerialPort();
        ...
        public class Port : IComparable<Port>
        {
            ...
            public Port([CanBeNull] string name, [CanBeNull] string desc)
            {
                Name = name;
                Desc = desc;
            }

            public override string ToString() => Desc ?? Name ?? string.Empty;

            public int CompareTo([CanBeNull] Port p)
            {
                if (p == null)
                    return 1;

                return string.CompareOrdinal(Name, p.Name);
            }
        }
        ...
        public static List<Port> GetDeviceList()
        {
            ...
            List<Win32DeviceMgmt.DeviceInfo> deviceList = Win32DeviceMgmt.GetAllCOMPorts();
            foreach (Win32DeviceMgmt.DeviceInfo device in deviceList)
            {
        ...
        public void Connect([CanBeNull] string portName)
        {
        ...
        {
            try { serialPort.DtrEnable = false; serialPort.Close(); } catch (Exception) { serialPort = new SerialPort(); }
        }
        ...
        public void SendData([NotNull] byte[] data, int size)
        {
        ...
        public int GetData([NotNull] byte[] data, int offset, int length) => serialPort.Read(data, offset, length);
    ...
    public class Win32DeviceMgmt
    {
        ...
        [NotNull]
        private static Guid[] GetClassGUIDs([CanBeNull] string className)
        {
            uint requiredSize;
